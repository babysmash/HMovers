...
- (void)initUI{
    ...
    [self.completeBtn setTitle:@"ok" forState:UIControlStateNormal];
    [self.completeBtn addTarget:self action:@selector(completeBtnClick) forControlEvents:UIControlEventTouchUpInside];
    [self.completeBtn setTitleColor:[UIColor orangeColor] forState:UIControlStateNormal];
    WS(ws);
...
- (void)setType:(PickerViewType)type{
    ...
    switch (type) {
        case PickerViewTypeSex:{
            self.titleLab.text = @"Sex selection";
            [self sexData];
        ...
        {
            self.titleLab.text = @"Select date";
           [self isDataPicker:YES];
            self.datePicke.datePickerMode = UIDatePickerModeDate;
            [self.datePicke setMinimumDate:[PickerView distanceYear:-120]];
...
#pragma mark 模拟数据
- (void)sexData{
    [self.array addObject:@[@"male",@"female"]];
}
- (void)heightData{
